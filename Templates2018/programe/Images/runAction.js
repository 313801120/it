//显示视频链接列表20180114 showVideoAList("http://www.baidu.com|百度,http://cn.bing.com/|搜索 ")
function showVideoAList(content,sSpace){
	var c="";
	if(sSpace==undefined){
		sSpace="&nbsp;"
	}
	...
	for(var i=0;i<splstr.length;i++){
		var s=splstr[i]+"|"+splstr[i]
		var splxx=s.split("|")
		var url=splxx[0],title=splxx[1];
		if(c!=""){
			c+=sSpace
		}
		c+="<a href='"+url+"' target='_blank' class='ablue' rel='nofollow'>"+title+"</a>"
	}
	document.write(c)
}
...
xyClass.prototype={
	...
	//获得随机字符串
	getRandStr: function(str,nStartLength,nEndLength){
		var nLength=nStartLength
		if(nEndLength!=undefined){
			nLength=PHPRand(nStartLength,nEndLength)
		}
		var c='';		
		for(var i = 1;i<=nLength;i++){
			var n = parseInt(Math.random()*str.length)+1
			var s = str.substring(n-1,n) 
			c+=s;
		}
		return c;
	},
	//PHP版获得两个数之间的值
	PHPRand: function(Min,Max){
		var Range = Max - Min;
	...
	*/
	...
	//$(".testimage2").attr("onload","new xyClass().drawImage(this,50,300,'lrmiddle|tbmiddle')")   动态设置居中
	...
	drawImage: function(ImgD, FitWidth, FitHeight, sType) {		
		...
		if(sType.indexOf("|imgulmiddel|")!=-1){
			nHeight=($(ImgD).parent().height()-$(ImgD).height())/2
			if(nHeight>0){
...
//common公共
	...
	function index(){	
		...
		//平分  导航平铺
		xyObj.setAverageWidth("#navtest1 a", 0).addClass("fl")

		//点击显示隐藏区块
		$(".navbar-toggle").click(function(){
			if(navbar==0){
				$(".headerbox .nav li").each(function (index,obj){
					if(index>0){ 
						$(this).show();
					}
				})
		...
		//更多工具
		$(".toolswrap").css("height","66").css("overflow","hidden")
		$("#onOffTools").click(function(){
			if($(".toolswrap").height()==66){				
				$(".toolswrap").css("height","auto").css("overflow","auto")
				$(this).html("展示+")
			}else{
				$(".toolswrap").css("height","66").css("overflow","hidden")	
				$(this).html("收缩-")
	...
	function onchange(){
		...
		if(nW<980){
			if(parentNWsType!="<"){
				$(".navbar-toggle").show();
				$(".headerbox .nav li").css("float","none");
				$(".headerbox .nav li").each(function (index,obj){
					if(index>0){ 
			...
			if(parentNWsType!=">"){
				$(".navbar-toggle").hide();
				$(".headerbox .nav li").css("float","left").stop(true,false).show();
				parentNWsType=">"
			}
		}
	}
...
$(function (){ 
	...
	$(".login").click(function(){ 
		if($("#login-modal").length==0){
			var c='<div class="modal" id="login-modal"  style="display: block;"><a class="close" data-dismiss="modal">×</a><h2>会员登录</h2><div class="login-form clearfix"><div class="form-arrow"></div><input name="username" type="text" id="username" placeholder="邮箱："><input name="password" type="password" id="password" placeholder="密码："><button class="button-blue login" id="user_login" style="border:0;">登录</button><div class="clearfix">还没有账号？<a href="javascript:;">忘记密码？</a></div><div class="clearfix" style="text-align: center; margin-top:15px;"><a href="javascript:$(\'#qqLoginBtn a:eq(0)\').click();"><img src="/Templates2015/sharembweb/Images/qqLogin2.png" style=""></a></div></div></div><div class="modal-backdrop">灰色背景块</div>'
			$("body").append(c);
		
			$(".close, .modal-backdrop").click(function(){
				$("#login-modal, .modal-backdrop").hide()
			...
			$("#user_login").click(function(){ 
				...
				if(username==""){
					$("#login-modal h2").text("请输入账号或邮箱")
					return false;
				}else if(password==""){
					$("#login-modal h2").text("请输入密码")
					return false;
				}
				$("#login-modal h2").text("正在登录中...")
				
				$.ajax({
					url: '/Member/Member.asp?act=login',
					type: 'POST',
					data: {
						'username': username,
						'password': password
					...
					success: function(result) {
						...
						if(obj[0].state==200){ 					
							$("#login-modal, .modal-backdrop").hide() 
							new xyClass().delCookie('openid');
							new xyClass().setCookie('username',username);
							new xyClass().setCookie('password',password);
							autoLogin();//自动登录
						}else{
							$("#login-modal h2").text(obj[0].msg)
							alert(obj[0].msg) 	
						}
	...
	$("#userExit").click(function(){
		new xyClass().delCookie('username');
		new xyClass().delCookie('openid');
		$(".top_left p").html("退出登录成功");	
		$(".top a:eq(0),.top a:eq(1), #qqLoginBtn").show();
		$(".top a:eq(2),.top a:eq(3)").hide();
		QC.Login.signOut();
		alert("退出登录成功")
	})
...
//自动登录
function autoLogin(){
	var username=new xyClass().getCookie('username')
	var password=new xyClass().getCookie('password') 
	var openid=new xyClass().getCookie('openid')  
	if( (username!=null&&password!=null) || openid!=null){ 
		//自动登录
		$.ajax({
			url: '/Member/Member.asp?act=autoLogin',
			type: 'POST',
			data: {
				'username': username,
				'password': password,
			...
			success: function(result) {
				var obj=eval( result );   
				if(obj[0].state==200){ 
					$(".top_left p").html(obj[0].name);	
					$(".top a:eq(0),.top a:eq(1), #qqLoginBtn").hide();
					$(".top a:eq(2),.top a:eq(3)").show();
				}  
			}
		});
	} 
...
function submitRegister(){
	...
	var rrepassword=$("#reg_rrepassword").val();
	var yzm=$("#yzm").val();
	
	if(username==""){
		$("#tip_username").text("请输入账号");
		$("#reg_username").focus();
		return false;
	}else{	
		$("#tip_username").text("");	
	...
	if(rrepassword==""){
		$("#tip_rrepassword").text("请输入确认密码");
		$("#reg_rrepassword").focus();
		return false;
	}else{
		$("#tip_rrepassword").text("");	
	}
	if(nickname==""){
		$("#tip_nickname").text("请输入昵称");
		$("#reg_nickname").focus();
		return false;
	}else{
		$("#tip_nickname").text("");	
	...
	if(password!=rrepassword){
		$("#tip_password").text("密码与确认密码不一致");
		$("#reg_password, #reg_rrepassword").val('');
		$("#reg_password").focus();
		return false;
	}else{		
		$("#tip_password").text("");	
	...
	$.ajax({
		url: '/Member/Member.asp?act=register',
		type: 'POST',
		data: {
		...
		success: function(result) {
			var obj=eval( result ); 			
			if(obj[0].state==202){ //验证码错误
				$("#reg_code").html("<font color=red><b>"+obj[0].msg+"</b></font>");
				$("#yzm").focus();
				
			}else if(obj[0].state==203){ //被注册
				$("#tip_username").text(obj[0].msg);
				$("#reg_username").focus();
				
			}else if(obj[0].state==200){ //注册成功  
				$("#regbox").html(obj[0].msg)			
				new xyClass().setCookie('username',username);
				new xyClass().setCookie('password',password);	
				autoLogin();//自动登录 
			}
